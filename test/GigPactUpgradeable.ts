...
import { GigPactUpgradeable, GigPactUpgradeable__factory } from "../typechain-types";
...
async function createAndSignRandomPact() {
  ...
  let signature = await employer.signMessage(ethers.utils.arrayify(await pact.contractDataHash(resultingEvent.pactid,signingDate)))
  let stake = (await pact.getAllPactData(resultingEvent.pactid))[0].payAmount
  await pact.employerSign(resultingEvent.pactid, signature, signingDate, { value: stake })
  signature = await employee.signMessage(ethers.utils.arrayify(await pact.contractDataHash(resultingEvent.pactid,signingDate)))
  await pact.employeeSign(resultingEvent.pactid, signature, signingDate)
...
const [testCreate, testSigning, testPactions, testdispute] = [true, true, true, false]
...
//       })
      ...
      it("should allow retracting offer before employee sign", async function () {
        ...
        let stake = (await pact.getAllPactData(resultingEvent.pactid))[0].payAmount
        await pact.employerSign(resultingEvent.pactid, signature, signingDate, { value: stake })
        await pact.retractOffer(resultingEvent.pactid)
  ...
  if (testPactions)
    describe("Pact Actions", function () {
      it("should allow starting a pact", async function () {
        ...
        await expect(pact.connect(employee).start(resultingEvent.pactid)).to.be.revertedWith("employer delegate only")
        await pact.delegate(resultingEvent.pactid, [thirdParty.address], true)
        await pact.connect(thirdParty).start(resultingEvent.pactid)
      })
