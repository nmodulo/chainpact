...
describe("WordPact", function () {
    ...
    if (testWithParticipants) describe("Adding and managing participants", function () {
        ...
        it("should allow authorized participants to withdraw", async function () {
            ...
            let tx = await (await pact.connect(participant1).withdraw(resultingEvent.uid, value/2)).wait()
            let balanceAfter = await ethers.provider.getBalance(participant1.address)
            expect(balanceAfter).to.eq(balanceBefore.add(value/2).sub(tx.gasUsed.mul(tx.effectiveGasPrice)))
            await pact.addParticipants(resultingEvent.uid, [{ addr: participant2.address, canVote: false, beneficiaryType: 0 }], [false])
            expect(pact.connect(participant2).withdraw(resultingEvent.uid, value/2)).to.be.revertedWith("Unauthorized")
        })
    })

    if(testVoting) describe("Test Voting", function(){
        it("should allow choosing and retrieving yes and no vote outcomes", async function() {
            let value = 1000
            let { resultingEvent } = await createNewPact(value)
            let party1 = { addr: participant1.address, canVote: false, beneficiaryType: 1 }  //YES
            let party2 = { addr: participant1.address, canVote: false, beneficiaryType: 2 }  //NO
        ...
        it("should allow starting vote with a fixed timeline, not allow voting after that", async function(){
            let value = 1000
            let { resultingEvent } = await createNewPact(value)
            let party1 = { addr: participant1.address, canVote: true, beneficiaryType: 1 }  //YES
            let party2 = { addr: participant2.address, canVote: true, beneficiaryType: 2 }  //NO
            let party3 = { addr: participant3.address, canVote: true, beneficiaryType: 2 }  //NO
            await pact.addParticipants(resultingEvent.uid, [party1, party2], [false, false])
        ...
        it("should disburse the yes and no vote beneficiaries after voting ends", async function(){
            let value = 1000
            let { resultingEvent } = await createNewPact(value)
            let party1 = { addr: participant1.address, canVote: true, beneficiaryType: 1 }  //YES Ben
            let party2 = { addr: participant2.address, canVote: true, beneficiaryType: 2 }  //NO  Ben
            let party3 = { addr: participant3.address, canVote: true, beneficiaryType: 1 }  //YES Ben
            let party4 = { addr: participant4.address, canVote: true, beneficiaryType: 1 }  //YES Ben
            ...
            console.log("Balances: Before after 0")
            console.log(balancesBefore[0], balancesAfter[0])

            expect(balancesAfter[0].sub(balancesBefore[0])).to.eq(Math.floor(value/3))
            expect(balancesAfter[1].sub(balancesBefore[1])).to.eq(0)
            expect(balancesAfter[2].sub(balancesBefore[2])).to.eq(Math.floor(value/3))
            expect(balancesAfter[3].sub(balancesBefore[3])).to.eq(Math.floor(value/3))
        })
        it("should allow refunding for result voted no, including non-participant pitch ins, even with no vote beneficiaries being separate")
        it("should not allow participants to be added during voting")
    })
});
